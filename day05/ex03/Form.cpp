...
{
	std::cout << "Constructor (Default) : Form called. (" << getName() << ")\n";
}
...
Form::Form(std::string name, int gradeToSign, int gradeToExecute) : _name(name), _requiredGradeToSign(gradeToSign), _requiredGradeToExecute(gradeToExecute), _signed(false)
{
	if (gradeToSign < 1 || gradeToExecute < 1)
		...
		std::cout << "Constructor (Classic) : Form called. (" << getName() << ")\n";
}
...
{
	std::cout << "Constructor (Copy) : Form called. (" << getName() << ")\n";
}

Form::~Form()
{
	std::cout << "Destructor : Form called. (" << getName() << ")\n";
}
